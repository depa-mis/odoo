...
class BioneCustomerReceipts(models.Model):

    _name = 'bione.customer.receipts'
    ...
    line_ids = fields.One2many('bione.customer.receipts.line', 'payment_id', string=u'รายการรับชำระ')
    other_ids = fields.One2many('bione.customer.receipts.other', 'payment_id', string=u'อื่นๆ')
    banktr_ids = fields.One2many('bione.customer.receipts.bank', 'payment_id', string=u'โอน')
    ...
    cheque_ids = fields.One2many('bione.cheque', 'customer_receipts_id', string=u'เช็ครับ')
    vat_ids = fields.One2many('bione.account.vat', 'customer_receipts_id', string=u'ภาษีขาย')
    wht_ids = fields.One2many('bione.wht', 'customer_receipts_id', string=u'ภาษีหัก ณ ที่จ่าย')
    ...
    @api.onchange('customer_id', 'date')
    def _onchange_customer_id_date(self):
        date_invoice = self.date
        if not date_invoice:
            date_invoice = fields.Date.context_today(self)
        if self.customer_id.property_payment_term_id.id:
            pterm = self.customer_id.property_payment_term_id
    ...
    def button_post(self):
        ...
        # revenue_income_account_id = int(params.get_param('bione_thai_account.revenue_income_account_id', default=False)) or False,
        # if revenue_income_account_id:
        #     move_data_vals = {
        #         'partner_id': self.customer_id.id,
        #         'invoice_id': False,
        #         'debit': 0.0,
        ...
        #         'payment_id': False,
        #         'account_id': revenue_income_account_id,
        #     }
        #     iml.append((0, 0, move_data_vals))
    ...
    def create(self, vals):
        ...
        vals['name'] = self.env['ir.sequence'].with_context(
                    ir_sequence_date=vals.get('date')).next_by_code('bione.customer.receipts')
        receipt_id = super(BioneCustomerReceipts, self.with_context(mail_create_nosubscribe=True)).create(vals)
        return receipt_id

    # @api.multi
    # @api.depends('name')
    # def _get_namevat(self):
    #     self.vat_ids.name = self.name

class BioneCustomerReceiptsLine(models.Model):
    _name = 'bione.customer.receipts.line'
    _description = 'Customer Receipts Line'
    ...
    payment_id = fields.Many2one('bione.customer.receipts', string=u'ขายสด')
    ...
    def _onchange_line_details(self):
        if not self.payment_id or not self.product_id:
            return
        onchange_res = self.product_id_change(
            self.product_id.id,
            self.payment_id.customer_id.id,
            self.price_unit)
    ...
    @api.multi
    def product_id_change(self, product_id, customer_id=False, price_unit=False):
        ...
        part = self.env['res.partner'].browse(customer_id)
        if part.lang:
...
class BioneCustomerReceiptsOther(models.Model):
    _name = 'bione.customer.receipts.other'
    _description = 'Customer Receipts Other'
...
class BioneCustomerReceiptsBank(models.Model):
    _name = 'bione.customer.receipts.bank'
    _description = 'Customer Receipts Bank'
