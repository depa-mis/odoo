...
class AccountPettyReceived(models.Model):
    ...
    def _default_journal(self):
        ...
        return int(params.get_param('bione_thai_account.petty_cash_receipt_journal_id', default=False)) or False
    ...
    journal_id = fields.Many2one("account.journal", "Journal",index=True, **REQUIRED, default=_default_journal)
    # journal_id = fields.Many2one("account.journal", "Journal",related='company_id.petty_cash_receipt_journal_id',index=True,**REQUIRED)
    ...
    # other_account_id = fields.Many2one('account.account', 'Other Account',related='company_id.cash_account_id',index=True,**REQUIRED)
    ...
    payment_id = fields.Many2one("account.payment", "Reference", domain=[('state', 'not in',['cancel']),('pay_received','=', True)],index=True,**OPTIONAL)
    ...
    def onchange_payment(self):
        ...
        # self.fund_id = self.payment_id.fund_id
    ...
    def action_cancel(self):
        ...
        # cash_move = self.env['account.cash.move'].search([('petty_id','=',self.id)])
        # print(cash_move)
        # if cash_move:
    ...
    def create(self, vals):
        vals['name'] = self.env['ir.sequence'].next_by_code('account.petty.received')
