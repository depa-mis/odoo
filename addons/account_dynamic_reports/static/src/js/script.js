odoo.define('account_dynamic_reports.DynamicTbMain', function (require) {
    ...
    var DynamicFrMain = AbstractAction.extend({
        ...
        events: {
            'click #filter_apply_button': 'update_with_filter', 'click #pdf': 'print_pdf', 'click #xlsx': 'print_xlsx', 'click .view-source': 'view_gl',
        },
    ...
    var DynamicGlMain = AbstractAction.extend({
        ...
        plot_data: function (initial_render = true) {
            ...
            self._rpc({
                ...
                if(self.filter_data.group_by_analytic == 0){
                        _.each(self.account_data, function (k, v) {
                        var formatOptions = {
                            currency_id: k.company_currency_id,
                            noSymbol: true,
                  ...
                  _.each(self.account_data, function (kk, vv) {
                        _.each(kk, function (k, v) {
                              var formatOptions = {
                                currency_id: k.company_currency_id,
                                noSymbol: true,
                            };
        ...
        view_move_line: function (event) {
            ...
            self._rpc({model: 'account.invoice', method: 'search_read',
                ...
                fields: ['id'], limit: 1, })
                    .then(function (record) {
                        if (record.length > 0) {
                            redirect_to_document('account.invoice', record[0].id);
                        } else {
    ...
    var DynamicTbMain = AbstractAction.extend({
        ...
        view_gl: function (event) {
            ...
            if (!domains.date_from && !domains.date_to && !domains.date_range) {
                domains.date_from = self.filter_data.date_from_cmp;
                domains.date_to = self.filter_data.date_to_cmp;
            }
            var gl_wizard_id = 0;
            self._rpc({
                model: 'ins.general.ledger',
    ...
    var DynamicPcMain = AbstractAction.extend({
        template: 'DynamicPcMain',
        events: {
            'click #filter_apply_button': 'update_with_filter',
            'click #pdf': 'print_pdf',
            'click #xlsx': 'print_xlsx',
            'click .view-source': 'view_petty_cash',
        },
        init: function (view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                console.log(data);
//                alert(data[2].total_payment_amount)
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'account_dynamic_reports.petty_cash',
                    'report_file': 'account_dynamic_reports.petty_cash',
                    'data': {'js_data': data ,'filter_data':data[0],'account_data':data[1],'line_total':data[2]},
                    'context': {'active_model': 'ins.petty.cash',
                        'landscape': 1,
                        'from_js': true
                    },
...
//                     alert(k);
                    var formatOptions = {
                     ...
                     k.receipt_amount = self.formatWithSign(k.receipt_amount, formatOptions, k.receipt_amount < 0 ? '-' : '');
                     k.payment_amount = self.formatWithSign(k.payment_amount, formatOptions, k.payment_amount < 0 ? '-' : '');
                    k.balance = self.formatWithSign(k.balance, formatOptions, k.balance < 0 ? '-' : '');
                    k.column_1 = self.formatWithSign(k.column_1, formatOptions, k.column_1 < 0 ? '-' : '');
                    k.column_2 = self.formatWithSign(k.column_2, formatOptions, k.column_2 < 0 ? '-' : '');
                    k.column_3 = self.formatWithSign(k.column_3, formatOptions, k.column_3 < 0 ? '-' : '');
                    k.column_4 = self.formatWithSign(k.column_4, formatOptions, k.column_4 < 0 ? '-' : '');
                    k.column_5 = self.formatWithSign(k.column_5, formatOptions, k.column_5 < 0 ? '-' : '');
                    k.column_6 = self.formatWithSign(k.column_6, formatOptions, k.column_6 < 0 ? '-' : '');
                    k.column_7 = self.formatWithSign(k.column_7, formatOptions, k.column_7 < 0 ? '-' : '');
                    k.column_8 = self.formatWithSign(k.column_8, formatOptions, k.column_8 < 0 ? '-' : '');
                    k.column_9 = self.formatWithSign(k.column_9, formatOptions, k.column_9 < 0 ? '-' : '');
                    k.column_10 = self.formatWithSign(k.column_10, formatOptions, k.column_10 < 0 ? '-' : '');
                    k.column_11 = self.formatWithSign(k.column_11, formatOptions, k.column_11 < 0 ? '-' : '');
                    k.column_12 = self.formatWithSign(k.column_12, formatOptions, k.column_12 < 0 ? '-' : '');
                });

                var formatOptions = {
                    currency_id: self.line_total.company_currency_id,
                    noSymbol: true,
                };

                self.line_total.total_payment_amount = self.formatWithSign(self.line_total.total_payment_amount, formatOptions, self.line_total.total_payment_amount < 0 ? '-' : '');
                self.line_total.total_balance_receipt = self.formatWithSign(self.line_total.total_balance_receipt, formatOptions, self.line_total.total_balance_receipt < 0 ? '-' : '');
                self.line_total.t_column_1 = self.formatWithSign(self.line_total.t_column_1, formatOptions, self.line_total.t_column_1 < 0 ? '-' : '');
                self.line_total.t_column_2 = self.formatWithSign(self.line_total.t_column_2, formatOptions, self.line_total.t_column_2 < 0 ? '-' : '');
                self.line_total.t_column_3 = self.formatWithSign(self.line_total.t_column_3, formatOptions, self.line_total.t_column_3 < 0 ? '-' : '');
                self.line_total.t_column_4 = self.formatWithSign(self.line_total.t_column_4, formatOptions, self.line_total.t_column_4 < 0 ? '-' : '');
                self.line_total.t_column_5 = self.formatWithSign(self.line_total.t_column_5, formatOptions, self.line_total.t_column_5 < 0 ? '-' : '');
                self.line_total.t_column_6 = self.formatWithSign(self.line_total.t_column_6, formatOptions, self.line_total.t_column_6 < 0 ? '-' : '');
                self.line_total.t_column_7 = self.formatWithSign(self.line_total.t_column_7, formatOptions, self.line_total.t_column_7 < 0 ? '-' : '');
                self.line_total.t_column_8 = self.formatWithSign(self.line_total.t_column_8, formatOptions, self.line_total.t_column_8 < 0 ? '-' : '');
                self.line_total.t_column_9 = self.formatWithSign(self.line_total.t_column_9, formatOptions, self.line_total.t_column_9 < 0 ? '-' : '');
                self.line_total.t_column_10 = self.formatWithSign(self.line_total.t_column_10, formatOptions, self.line_total.t_column_10 < 0 ? '-' : '');
                self.line_total.t_column_11 = self.formatWithSign(self.line_total.t_column_11, formatOptions, self.line_total.t_column_11 < 0 ? '-' : '');
                self.line_total.t_column_12 = self.formatWithSign(self.line_total.t_column_12, formatOptions, self.line_total.t_column_12 < 0 ? '-' : '');

                self.filter_data.date_from = field_utils.format.date(field_utils.parse.date(self.filter_data.date_from, {}, {isUTC: true}));
                self.filter_data.date_to = field_utils.format.date(field_utils.parse.date(self.filter_data.date_to, {}, {isUTC: true}));
//                alert(self.filter_data.date_to);
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionPc', {
                        filter_data: self.filter_data,
                    }));
                    self.$el.find('#date_from').datepicker({dateFormat: 'dd-mm-yy'});
                    self.$el.find('#date_to').datepicker({dateFormat: 'dd-mm-yy'});
...
//                alert(self.filter_data.date_to);
                  ...
                  self.$('.py-data-container').html(QWeb.render('DataSectionPc', {
                    account_data: self.account_data,
                    filter_data: self.filter_data,
        ...
        view_petty_cash: function (event) {
            event.preventDefault();
            var self = this;
            var domains = {}
//            var domains = {account_ids: [$(event.currentTarget).data('account-id')],
...
//            }
    ...
    var DynamicAvMain = AbstractAction.extend({
        template: 'DynamicAvMain',
        events: {
            'click #filter_apply_button': 'update_with_filter',
            'click #pdf': 'print_pdf',
            'click #xlsx': 'print_xlsx',
            'click .view-source': 'view_advances',
        },
        init: function (view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'account_dynamic_reports.advances',
                    'report_file': 'account_dynamic_reports.advances',
                    'data': {'js_data': data ,'filter_data':data[0],'account_data':data[1],'line_total':data[2]},
                    'context': {'active_model': 'ins.advances',
                        'landscape': 1,
                        'from_js': true
                    },
            ...
            self._rpc({
                ...
                _.each(self.account_data, function (k, v) {
                    ...
                    var formatOptions = {
                     ...
                     k.contract_amount = self.formatWithSign(k.contract_amount, formatOptions, k.contract_amount < 0 ? '-' : '');
                     k.amount_clear = self.formatWithSign(k.amount_clear, formatOptions, k.amount_clear < 0 ? '-' : '');
                ...
                self.line_total.total_contract_amount = self.formatWithSign(self.line_total.total_contract_amount, formatOptions, self.line_total.total_contract_amount < 0 ? '-' : '');

                self.filter_data.date_from = field_utils.format.date(field_utils.parse.date(self.filter_data.date_from, {}, {isUTC: true}));
                self.filter_data.date_to = field_utils.format.date(field_utils.parse.date(self.filter_data.date_to, {}, {isUTC: true}));

                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionAv', {
                        filter_data: self.filter_data,
                    }));
                    self.$el.find('#date_from').datepicker({dateFormat: 'dd-mm-yy'});
                    self.$el.find('#date_to').datepicker({dateFormat: 'dd-mm-yy'});
                  ...
                  self.$('.py-data-container').html(QWeb.render('DataSectionAv', {
                    account_data: self.account_data,
                    filter_data: self.filter_data,
                    line_total : self.line_total,
                }));
    ...
    core.action_registry.add('dynamic.petty.cash', DynamicPcMain);
    core.action_registry.add('dynamic.advances', DynamicAvMain);
});
