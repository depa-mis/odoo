...
class InsTrialBalance(models.TransientModel):
    ...
    def prepare_hierarchy(self, move_lines):
        ...
        if move_lines:
            ...
            for line in move_lines:
                ...
                tmp = q.copy()
                tmp.update({'code': str(tmp['code']), 'parent': ' a'.join(final_parent), 'dummy': False,
                            'indent_list': [1, 2, 3, 4], })
                hirarchy_list.append(tmp)
    ...
    def process_data(self, data):
        if data:
            ...
            if self.show_gf_code:
                account_gf_code_ids = self.env['gf.code'].search([])
                gf_code = []
                for account_code in account_gf_code_ids:
                    gf_code.append(account_code.gf_code_text)
                account_ids = self.env['account.account'].search([('gf_code', 'in', gf_code)], order="id asc")
            ...
            total_end_bal = 0.0
            if self.show_gf_code:
                move_lines = {x.gf_code: {'name': x.gf_id.name, 'gf_code': x.gf_code, 'code': x.gf_code, 'id': x.id,
                ...
                for account in account_ids:
                    ...
                    move_lines[account.gf_code]['initial_balance'] += init_blns['initial_balance']
                    move_lines[account.gf_code]['initial_debit'] += init_blns['initial_debit']
                    move_lines[account.gf_code]['initial_credit'] += init_blns['initial_credit']

                    if account.user_type_id.include_initial_balance and self.strict_range:
                        move_lines[account.gf_code]['initial_balance'] = 0.0
                        move_lines[account.gf_code]['initial_debit'] = 0.0
                        move_lines[account.gf_code]['initial_credit'] = 0.0
                    ...
                    cre = op['credit']
                    bln = op['balance']
                    move_lines[account.gf_code]['debit'] += deb
                    move_lines[account.gf_code]['credit'] += cre
                    move_lines[account.gf_code]['balance'] += bln

                    end_blns = init_blns['initial_balance'] + bln
                    end_cr = init_blns['initial_credit'] + cre
                    end_dr = init_blns['initial_debit'] + deb

                    move_lines[account.gf_code]['ending_balance'] += end_blns
                    move_lines[account.gf_code]['ending_credit'] += end_cr
                    move_lines[account.gf_code]['ending_debit'] += end_dr


                    if (data.get('display_accounts') == 'balance_not_zero') and (self.show_gf_code == False):
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'show_gf_code': self.show_gf_code,
