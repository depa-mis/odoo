...
class InsGeneralLedger(models.TransientModel):
    ...
    group_by_analytic = fields.Boolean(
        string='Group By analytic Account', default=False
    )
    ...
    def process_filters(self):
        ...
        filters['group_by_analytic'] = data.get('group_by_analytic')
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                SELECT
                    ...
                    anl.code as analytic_name,
                    ct.code as cost_center
                FROM account_move_line l
                JOIN account_move m ON (l.move_id=m.id)
                JOIN account_account a ON (l.account_id=a.id)
                LEFT JOIN account_analytic_account anl ON (l.analytic_account_id=anl.id)
                ...
                LEFT JOIN res_partner p ON (l.partner_id=p.id)
                JOIN account_journal j ON (l.journal_id=j.id)
                LEFT JOIN account_cost_center ct ON (l.cost_center_id=ct.id)
                WHERE %s
                GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name, anl.code , m.narration, ct.code
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def process_data(self):
        ...
        # print(data.get('analytic_ids', []))
        # print('analytic_ids---->', account_move_line)

        # print('analytic_ids---->', analytic_ids)

        print('analytic_ids---->',  data.get('analytic_ids', []))
        # if data.get('group_by_analytic'):
        if data.get('group_by_analytic'):
            if not data.get('analytic_ids'):
                ...
                account_move_line = self.env['account.move.line'].search(
                    [('analytic_account_id', 'in', data.get('analytic_ids',[]))])
            ...
            for move_line in account_move_line:
                ...
                #     print(account.id, move_line.account_id.id)
                #     print('x---->', account.name)
                #     print('account_id---->', move_line.account_id.id)
                #     if account.code == move_line.account_id.code:
                ...
                if data.get('initial_balance'):
                    ...
                    WHERE_FULL = WHERE + " AND l.date >= '%s'" % data.get(
                        'date_from') + " AND l.date <= '%s'" % data.get(
                        'date_to')
                ...
                if move_line.analytic_account_id.id in analytic:
                    move_lines = {}
                else:
                    analytic.append(move_line.analytic_account_id.id)
                    analytic_move_lines[move_line.analytic_account_id.name] = move_lines


            print('analytic_move_lines---->', analytic_move_lines)

            return analytic_move_lines
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'group_by_analytic': self.group_by_analytic,
