...
class kpi_group_contribute_lines(models.Model):
    _name = 'kpi_group_contribute_lines'
    ...
    group_contribute_lines_id = fields.Many2one(
        'kpi_main',
        string="group contribute id"
    )
    kpi_group_contribute_evaluation_lines_ids = fields.One2many(
        "kpi_group_contribute_evaluation_lines",
        "kpi_group_contribute_evaluation_lines_id",
    ...
    group_contribute_attachment_ids = fields.Many2many(
        "ir.attachment",
        "group_contribute_lines_attachment_rel",
        "group_contribute_attachment_lines_id",
        "ir_attachment_id",
        string='ไฟล์แนบ',
        # required=True
    )

    @api.depends('kpi_group_contribute_evaluation_lines_ids')
    def _kpi_show_score_inline(self):
        for rec in self:
            for line in rec.kpi_group_contribute_evaluation_lines_ids:
...
class kpi_group_contribute_evaluation_lines(models.Model):
    _name = 'kpi_group_contribute_evaluation_lines'

    kpi_group_contribute_evaluation_lines_id = fields.Many2one(
    ...
    @api.depends('kpi_group_contribute_evaluation_lines_id.group_contribute_lines_id.state')
    def _compute_is_latest_evaluator(self):
        for rec in self:
            state = rec.kpi_group_contribute_evaluation_lines_id.group_contribute_lines_id.state
            if state == 'evaluating':
    ...
    def onchange_score(self):
        for rec in self:
            total_score = rec.kpi_group_contribute_evaluation_lines_id.contribution_score
