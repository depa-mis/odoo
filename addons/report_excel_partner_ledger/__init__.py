...
def pre_init_check(cr):
    ...
    # if int(''.join([str(100+int(d)) for d in  report_excel.installed_version.split('.')[2:]])) < 101103112:
    #     raise Warning('Module support "Professional Reports Excel" module starting from Version 1.3.12, found Version {}. Please update the module "Professional Reports Excel".'.format('.'.join(report_excel.installed_version.split('.')[2:])))
    return True