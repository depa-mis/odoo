...
class internalDocumentAnnounce(models.Model):
    _name = 'document.internal.announce'
    _description = 'สารบรรณ หนังสือภายใน ประกาศ'
    ...
    setting_line_ids = fields.One2many('document.internal.main.setting.line','setting_id', string='Document main line')
    ...
    def geturl(self):
        self.update({
            'subject' : self.env['ir.config_parameter'].get_param('web.base.url')
        })
        raise ValidationError(_(self.env['ir.config_parameter'].sudo().get_param('web.base.url')))
        
class  InternalDocumentAnnounceSettingLine(models.Model):
    _name = 'document.internal.announce.setting.line'
    _order='status,sequence,id'

    setting_id = fields.Many2one('document.internal.announce', string="document announce")
