...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        data = data[0]
        self.row_pos += 3
        if data.get('group_by_analytic',False) == False:
            if filter.get('include_details', False):
                self.sheet.write_string(self.row_pos, 0, _('Date'),
                                        self.format_header)
                self.sheet.write_string(self.row_pos, 1, _('Move'),
                                        self.format_header)
                self.sheet.write_string(self.row_pos, 2, _('JRNL'),
                                        self.format_header)
                ...
                self.sheet.write_string(self.row_pos, 7, _('Balance'),
                                        self.format_header)
                self.sheet.write_string(self.row_pos, 8, _('Note'),
                                        self.format_header)
                self.sheet.write_string(self.row_pos, 9, _('Analytic account'),
                                        self.format_header)
                self.sheet.write_string(self.row_pos, 10, _('Cost Center'),
                                        self.format_header)
            else:
            ...
            if acc_lines:
                for line in acc_lines:
                    ...
                    self.sheet.write_string(self.row_pos, 0, acc_lines[line].get('code'), self.line_header)
                    self.sheet.write_string(self.row_pos, 1, acc_lines[line].get('name'), self.line_header)

                    # self.sheet.write_number(self.row_pos, 1, self.row_pos, 3, acc_lines[line].get('name'), self.line_header)
                    ...
                    if filter.get('include_details', False):
                        ...
                        for sub_line in sub_lines:
                            ...
                            if sub_line.get('move_name') == 'Initial Balance':
                                self.row_pos += 1
                                self.sheet.write_string(self.row_pos, 8, sub_line.get('move_name'),
                                ...
                                self.sheet.write_string(self.row_pos, 1, sub_line.get('move_name'),
                                                        self.line_left)
                                self.sheet.write_string(self.row_pos, 2, sub_line.get('lcode'),
                                                        self.line_left)
                                self.sheet.write_string(self.row_pos, 3, sub_line.get('partner_name') or '',
                                                        self.line_left)
                                ...
                                #############################################################
                                self.sheet.write_string(self.row_pos, 8, sub_line.get('narration') or '',
                                                        self.line_left)
                                self.sheet.write_string(self.row_pos, 9, sub_line.get('analytic_name') or '',
                                                        self.line_left)
                                self.sheet.write_string(self.row_pos, 10, sub_line.get('cost_center') or '',
                                                        self.line_left)
                                #############################################################
            ...
            analytic = acc_lines
            if analytic:
                for analytic in acc_lines:
                    self.row_pos += 1
                    self.sheet.write_string(self.row_pos, 0, analytic, self.line_header_left)
                    for line in acc_lines[analytic]:
                        self.row_pos += 1
                        self.sheet.write_string(self.row_pos, 1, acc_lines[analytic][line].get('name'), self.line_header)
                        self.sheet.merge_range(self.row_pos, 0, self.row_pos, 4, '   ' + acc_lines[analytic][line].get('code') + ' - ' + acc_lines[analytic][line].get('name'), self.line_header_left)
                        # self.sheet.write_string(self.row_pos, 0, acc_lines[analytic][line].get('code'), self.line_header)
                        # self.sheet.write_string(self.row_pos, 1, acc_lines[analytic][line].get('name'), self.line_header)
                        self.sheet.write_number(self.row_pos, 5, float(acc_lines[analytic][line].get('debit')), self.line_header)
                        self.sheet.write_number(self.row_pos, 6, float(acc_lines[analytic][line].get('credit')), self.line_header)
                        self.sheet.write_number(self.row_pos, 7, float(acc_lines[analytic][line].get('balance')), self.line_header)
                        if filter.get('include_details', False):
                            account_id = acc_lines[analytic][line].get('id')
                            count, offset, sub_lines = self.record.build_detailed_move_lines(offset=0, account=account_id,
                                                                                             fetch_range=1000000)
                            for sub_line in sub_lines:
                                if sub_line.get('move_name') == 'Initial Balance':
                                    ...
                                    self.sheet.write_string(self.row_pos, 8, sub_line.get('move_name'),
                                                            self.line_header_light_initial)
                                    self.sheet.write_number(self.row_pos, 5, float(acc_lines[analytic][line].get('debit')),
                                                            self.line_header_light_initial)
                                    self.sheet.write_number(self.row_pos, 6, float(acc_lines[analytic][line].get('credit')),
                                                            self.line_header_light_initial)
                                    self.sheet.write_number(self.row_pos, 7, float(acc_lines[analytic][line].get('balance')),
                                                            self.line_header_light_initial)
                                elif sub_line.get('move_name') not in ['Initial Balance','Ending Balance']:
                                    self.row_pos += 1
                                    self.sheet.write_datetime(self.row_pos, 0, self.convert_to_date(sub_line.get('ldate')),
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(0, 0, 0, 10, 'General Ledger'+' - '+data[0]['company_id'][1], self.format_title)

            self.dateformat = self.env.user.lang
            filters, account_lines = record.get_report_datas()

            self.sheet.merge_range(1, 5, 1, 10, _('Date from ') + str( filters['date_from'] or ''), self.line_left)
            self.sheet.merge_range(2, 5, 2, 10, _('Date to ') + str(filters['date_to'] or ''), self.line_left)
