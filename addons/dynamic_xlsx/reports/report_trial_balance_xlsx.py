...
class InsTrialBalanceXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, acc_lines, retained, subtotal, filter):
        ...
        self.sheet.write_string(self.row_pos, 0, _('Code'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 1, _('Account'),
                                self.format_header)
        ...
        self.sheet.write_string(self.row_pos, 7, _('Balance'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 8, _('Debit'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 9, _('Credit'),
                                self.format_header)
        ...
        if acc_lines:
            if not filter.get('show_hierarchy'):
                for line in acc_lines: # Normal lines
                    self.row_pos += 1
                    self.sheet.write_string(self.row_pos, 0,  acc_lines[line].get('code'), self.line_header_light)
                    self.sheet.write_string(self.row_pos, 1,  acc_lines[line].get('name'), self.line_header_light)
                    ...
                    self.sheet.write_number(self.row_pos, 5, float(acc_lines[line].get('debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 6, float(acc_lines[line].get('credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 7, float(acc_lines[line].get('balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 8, float(acc_lines[line].get('ending_debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 9, float(acc_lines[line].get('ending_credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 10, float(acc_lines[line].get('ending_balance')), self.line_header_highlight)
            else:
                for line in acc_lines: # Normal lines
                    self.row_pos += 1
                    ...
                    if line.get('dummy'):
                        ...
                        self.sheet.write_string(self.row_pos, 0, blank_space + line.get('code'), self.line_header_light_left)
                        self.sheet.write_string(self.row_pos, 1, line.get('name'), self.line_header_light_left)
                    ...
                    self.sheet.write_number(self.row_pos, 5, float(line.get('debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 6, float(line.get('credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 7, float(line.get('balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 8, float(line.get('ending_debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 9, float(line.get('ending_credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 10, float(line.get('ending_balance')), self.line_header_highlight)
            ...
            self.sheet.write_number(self.row_pos, 5, float(subtotal['SUBTOTAL'].get('debit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 6, float(subtotal['SUBTOTAL'].get('credit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 7, float(subtotal['SUBTOTAL'].get('balance')), self.line_header_total)
            self.sheet.write_number(self.row_pos, 8, float(subtotal['SUBTOTAL'].get('ending_debit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 9, float(subtotal['SUBTOTAL'].get('ending_credit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 10, float(subtotal['SUBTOTAL'].get('ending_balance')), self.line_header_total)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.row_pos = 0
        self.row_pos_2 = 0
        self.sheet = workbook.add_worksheet('Trial Balance')
