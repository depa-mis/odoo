...
class AccountAsset(models.Model):
    ...
    purchase_value = fields.Float(
        string='Residual Begin Value', required=True, readonly=True,
        states={'draft': [('readonly', False)]},
    ...
    purchase_paid_value = fields.Float(
        'Purchase Value',
        required=True,
        states={'draft': [('readonly', False)]}
    )
    ...
    @api.depends('depreciation_base',
                 ...
                 'depreciation_line_ids.is_not_create_move',)
    def _compute_depreciation(self):
        for asset in self:
            lines = asset.depreciation_line_ids.filtered(
                lambda l: l.type in ('depreciate', 'remove') and
                (l.init_entry or l.move_check or l.is_not_create_move))
            ...
            for li, line_date in enumerate(line_dates):
                ...
                if i == 0 and li == 0:
                    if entry.get('day_amount') > 0.0:
                        amount_per_year = self.depreciation_base / self.method_number
                        cy_year = calendar.isleap(line_date.year) and 366 or 365
                        amount = round(amount_per_year * line_days / cy_year, digits)
                    else:
    ...
    def _compute_entries(self, date_end, check_triggers=False):
        ...
        depreciations_les = self.env['account.asset.line'].search([
            ('asset_id', 'in', self.ids),
            ('type', '=', 'depreciate'),
            ('init_entry', '=', False),
            ('line_date', '<=', date_end),
        ...
        for line in  depreciations_les:
            line.is_not_create_move = True
