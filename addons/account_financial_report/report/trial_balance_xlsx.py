...
class TrialBalanceXslx(models.AbstractModel):
    ...
    def _get_report_columns(self, report):
        if not report.show_partner_details:
            if report.gf_code == True:
                res = {
                    0: {'header': _('Code'), 'field': 'gf_code', 'width': 10},
    ...
    def _generate_report_content(self, workbook, report):
        ...
        for account in report.account_ids.filtered(lambda a: not a.hide_line):
            sum_initial_balance += account.initial_balance
            sum_debit += account.debit
            sum_credit += account.credit
            sum_period_balance += account.period_balance
            sum_final_balance += account.final_balance
            if not report.show_partner_details:
                ...
                # Line break
                self.row_pos += 2
            self.sheet.write(self.row_pos, 0, '', self.format_header_amount)
            self.sheet.write(self.row_pos, 1, 'ยอดรวม', self.format_header_amount)
            self.sheet.write_number(self.row_pos, 2, sum_initial_balance,
                                    self.format_header_amount)
            self.sheet.write_number(self.row_pos, 3, sum_debit,
                                    self.format_header_amount)
            self.sheet.write_number(self.row_pos, 4, sum_credit,
                                    self.format_header_amount)
            self.sheet.write_number(self.row_pos, 5, sum_period_balance,
                                    self.format_header_amount)
            self.sheet.write_number(self.row_pos, 6, sum_final_balance,
                                    self.format_header_amount)
