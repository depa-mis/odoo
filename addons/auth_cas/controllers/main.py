...
class Controller(http.Controller):
    ...
    def cas_authenticate(self):
        ...
        # cas_login = config.get('host') + '' + config.get('port') + \
        #     '/cas/login?service=' + service_url
        casUrl, serviceUrl, dbName = Home._get_config_url()
        service_url = serviceUrl

        cas_login = config.get('host') + '' + '/cas/login?service=' + service_url
        # cas_login = \
    ...
    def cas_authenticate_user(
        ...
        if cas_port == -1:
            ...
            cas_server = url_server.scheme + '://' + url_server.netloc + '' + url_server.path
...
class Home(main.Home):
    @http.route('/web/auth_cas/callback', type="http", auth="none")
    def auth_callback(self, ticket):
        _logger.info('cas auth test')
        ...
        _logger.info('cas authenticated')
    ...
    def _get_config_url():
        ...
        cas_url = config['host']
        # service_url = request.httprequest.url_root
        service_url = config.get('odoo_url') + '/web/auth_cas/callback'
