odoo.define('badge_on_menu', function(require) {
    ...
    Menu.include({
        ...
        start: function() {
            ...
            var onloadbadge = setInterval(function(){
                // if($('.o_menu_apps .dropdown .show').length == 0){
                if(($('.menu_count_lvl_1').length > 0) || $('.menu_count_lvl_2').length > 0){
                    clearInterval(onloadbadge);
                } else {
                    self.on_load_badge();
                }
                // }
            }, 1000);
            // var onloadbadge = setTimeout(function(){
            //     if($('.o_menu_apps .dropdown .show').length == 0){
            //         self.on_load_badge();
            //     }
            // }, 5000);
        ...
        on_load_badge: function() {
            ...
            //     });
            // })
            _.each($(".o_menu_sections").find('.o_menu_header_lvl_1[data-menu-xmlid="pfb_saraban.internal_document_menu"], .o_menu_header_lvl_1[data-menu-xmlid="pfb_saraban.receive_document_menu"]'), function($item) {
                var act_vals = $item.text;
                if (act_vals) {
                    var postData = new FormData();
                    if (core.csrf_token) {
                    ...
                    var def = $.ajax({
                        ...
                        if (count["count"] == 0) {
                            ...
                            $item.text = count["menu_label"]
                            var html_to_insert = "&nbsp;&nbsp;<span class='dd menu_count_lvl_1'>" + count["count"] + "</span>";
                            $item.innerHTML += html_to_insert;
                        }
                    });
                }
            });
            _.each($(".o_menu_sections").find('.o_menu_entry_lvl_2[data-menu-xmlid="pfb_saraban.internal_waiteing_menu"], .o_menu_entry_lvl_2[data-menu-xmlid="pfb_saraban.receive_waithing_menu"]'), function($item) {
                var act_vals = $item.href.indexOf($item.href.split('&')[1])
                if (act_vals > -1 || act_vals > -1) {
                    var postData = new FormData();
                    if (core.csrf_token) {
                    ...
                    var def = $.ajax({
                        ...
                        if (count["count"] == 0) {
                            ...
                            $item.text = count["menu_label"]
                            var html_to_insert = "&nbsp;&nbsp;<span class='dd menu_count_lvl_2'> " + count["count"] + "</span>";
                            $item.innerHTML += html_to_insert;
                        }
                    });
