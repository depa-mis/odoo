...
class UploadAttachment(http.Controller):
    ...
    @http.route(['/get_badge_count_menu'], type='http', auth='user', website=True)
    def get_badge_count_menu(self,  **post):
        ...
        if act_id1 in href_link:
            document_internal_values = 0
            all_rec = request.env['document.internal.main'].search([
                ('setting_line_ids.status_approve', '=', '0'),
                ('waiting_line_ids.employee_id.user_id', '=', request.env.uid),
                # ('setting_line_ids.status_approve', '=', '0'),
                # ('setting_line_ids.employee_id.user_id', '=', request.env.uid),
                ('state', '!=', 'done')]
            )
            # cnt_values = len(all_rec)
            document_internal_values = len(all_rec)
            return json.dumps({'count': document_internal_values,'menu_label':name})
            ...
            # cnt_values = len(all_rec)
            receive_values = len(all_rec)
            return json.dumps({'count': receive_values,'menu_label':name})
        else:
    ...
    def get_badge_count_head(self,  **post):
        ...
        if "หนังสือภายใน" in text:
            ...
            document_internal_values = len(all_rec)
            # cnt_values = len(all_rec)
            return json.dumps({'count': document_internal_values,'menu_label':"หนังสือภายใน"})
        # elif(text.find("หนังสือรับ")>0):
        elif "หนังสือรับ" in text:
            receive_values = 0
            all_rec = request.env['receive.document.main'].search([
                ('setting_line_ids.status_approve', '=', '0'),
                ('waiting_line_ids.employee_id.user_id', '=', request.env.uid),
            ...
            # cnt_values = len(all_rec)
            receive_values = len(all_rec)
            return json.dumps({'count': receive_values,'menu_label':"หนังสือรับ"})
        else:
