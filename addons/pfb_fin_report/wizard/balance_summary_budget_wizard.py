...
class BalanceSummaryBudgetWizard(models.TransientModel):
    _name = 'balance.summary.budget.report.wizard'
    ...
    def button_export_html(self):
        self.ensure_one()
        action = self.env.ref(
            'pfb_fin_report.action_summary_budget_report_html')
        vals = action.read()[0]
        context = vals.get('context', {})
        if isinstance(context, pycompat.string_types):
            context = safe_eval(context)
        model = self.env['report.summary.budget.report']
        report = model.create(self._prepare_balance_summary_budget_report())
    ...
    def _prepare_balance_summary_budget_report(self):
        self.ensure_one()
        return {
