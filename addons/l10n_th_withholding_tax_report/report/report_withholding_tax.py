...
class WithHoldingTaxReport(models.TransientModel):
    ...
    def print_report(self, report_type='qweb'):
        ...
        if report_type == 'xlsx':
            ...
            print(test014444444444444444444)
    ...
    def _compute_results(self):
        self.ensure_one()
        Result = self.env['bione.wht.line']
        domain = []
        if self.income_tax_form:
            domain += [('wht_id.wht_kind', '=', self.income_tax_form)]
        if self.date_from:
            domain += [('wht_id.date_doc', '>=', self.date_from)]
        if self.date_to:
            domain += [('wht_id.date_doc', '<=', self.date_to)]
